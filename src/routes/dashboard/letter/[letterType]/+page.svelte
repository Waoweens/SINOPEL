<script lang="ts">
	...
	const letterType = data.letter.type;
			...
			<tbody>
				<Collection ref={'users'} let:data>
					{#each $rows as row}
						<tr>
							<td>
								<a class="btn-icon variant-filled-secondary !rounded-md" href="/dashboard/letter/{letterType}/edit/{row.id}">
							...
							<td>
								{data.find((obj) => obj.id == row.created.user)?.displayName ?? 'Akun tidak valid'}
								<br />
							...
							<td>
								{data.find((obj) => obj.id == row.modified.user)?.displayName ?? 'Akun tidak valid'}
								<br />
